...
#
#   ci/nested-shell.sh start [--extension DIR]... [--state DIR] [--monitor WxH]
#   ci/nested-shell.sh stop  [--state DIR]
#   ci/nested-shell.sh env   [--state DIR]     # prints shell-eval'able env for the session
#
# Taken from ideas/gnome-tasks/tools/nested-shell.sh with its gnome-tasks specifics dropped.
# Reused rather than rewritten precisely because it is already known to work — and because of
# the dconf trap documented in start(), which it guards against: a private bus is NOT enough to
# keep a nested session's settings out of the real desktop's dconf database.
#
# The environment for talking to the nested session is written to $STATE/env:
#
#   source build/smoke/env
#   gnome-extensions list            # the nested Shell's extensions, not yours
set -euo pipefail

# Under /tmp, and it has to be: $STATE/bus is a Unix socket, whose path cannot exceed about 107
# characters, and a checkout inside a deep worktree blows that on its own ("Socket name too
# long"). Nothing durable lives here — the session's state is scratch, and the artefacts worth
# keeping (screenshots) are written into the idea folder by the caller.
...
# libreoffice on Ubuntu) open $XDG_RUNTIME_DIR/wayland-N. A display called "aideas-nested" is
...
# Absolute, always: these paths become XDG_CONFIG_HOME and friends for the nested session, and
# a relative XDG_CONFIG_HOME means dconf writes go nowhere — it times out committing and the
# leak check below then (correctly) refuses to continue.
mkdir -p "$STATE"
STATE="$(cd "$STATE" && pwd)"
...
export AIDEAS_NESTED_STATE=$STATE
EOF
