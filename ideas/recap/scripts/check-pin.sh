#!/usr/bin/env bash
# recap's sources are pinned twice: as the `upstream/` git submodule (what you edit) and as
# the `recap-src` flake input (what `nix flake check` builds and tests). If those drift, CI
# here would happily test a commit nobody is working on, so assert they match.
#
# Run from ideas/recap, inside `nix develop` (needs git and jq).
set -euo pipefail
...
# from the index rather than the checkout, so a bump that has been staged but not yet
...
flake_rev="$(jq -r '.nodes["recap-src"].locked.rev' flake.lock)"
if [ -z "$flake_rev" ] || [ "$flake_rev" = "null" ]; then
    echo "flake.lock has no locked rev for the recap-src input" >&2
    exit 1
fi

echo "submodule upstream/ -> $submodule_rev"
echo "flake input recap-src -> $flake_rev"
...
Point them at the same commit:
  ...
  nix flake lock --override-input recap-src github:gortazar/recap/<rev>
EOF
    exit 1
fi
