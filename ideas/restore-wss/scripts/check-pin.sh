...
# edit) and as the `restore-wss-src` flake input (what `nix flake check` tests). If those
# drift, CI here would happily test a commit nobody is working on, so assert they
# match.
#
# Run from ideas/restore-wss, inside `nix develop` (needs git and jq).
set -euo pipefail
...
flake_rev="$(jq -r '.nodes["restore-wss-src"].locked.rev' flake.lock)"
if [ -z "$flake_rev" ] || [ "$flake_rev" = "null" ]; then
    echo "flake.lock has no locked rev for the restore-wss-src input" >&2
    exit 1
fi

echo "submodule upstream/ -> $submodule_rev"
echo "flake input restore-wss-src -> $flake_rev"
...
Point them at the same commit:
  ...
  nix flake lock --override-input restore-wss-src github:gortazar/restore-wss/<rev>
EOF
    exit 1
fi
