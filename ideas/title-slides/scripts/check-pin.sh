...
# edit) and as the `title-slides-src` flake input (what `nix flake check` tests). If
# those drift, CI here would happily test a commit nobody is working on, so assert
# they match.
#
# Run from ideas/title-slides, inside `nix develop` (needs git and jq).
set -euo pipefail
...
flake_rev="$(jq -r '.nodes["title-slides-src"].locked.rev' flake.lock)"
if [ -z "$flake_rev" ] || [ "$flake_rev" = "null" ]; then
    echo "flake.lock has no locked rev for the title-slides-src input" >&2
    exit 1
fi

echo "submodule upstream/ -> $submodule_rev"
echo "flake input title-slides-src -> $flake_rev"
...
Point them at the same commit:
  ...
  nix flake lock --override-input title-slides-src github:gortazar/title-slides/<rev>
EOF
    exit 1
fi
