...
5. [restore-wss](ideas/restore-wss) - One open BLOCKER on `gortazar_restore-wss` is neither fixed nor documented while `STATUS.md` says `status: done`. Minor update. It is
...
6. [gnome-tasks](ideas/gnome-tasks) - `STATUS.md` asserts something untrue about the tree, and `STATUS.md` is the only report anyone reads to judge an idea. Minor update. Line 74
...
7. [title-slides](ideas/title-slides) - Two documentation gaps a fleet audit confirmed, neither of which affects the code. Minor update. (1) `ideas/title-slides/STATUS.md` never
...
25. [orchestrator](ideas/orchestrator) - A quality gate on `main` will reject the orchestrator's own end-of-cycle pushes, and the rescue path it falls back to keeps the work only on this (finished 2026-09-15, v1.6)
